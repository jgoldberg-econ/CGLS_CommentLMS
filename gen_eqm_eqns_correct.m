...
% This code is used to produce the red dashed line in our Figure 3. 
...
elseif flag==2 % quadratic costs, different cost function for leader and follower
    ...
    for i=2:n
        if i>2
            out(n+i) = (pivec(n+1+i)-pivec(n+i))/2/c^2 + a*(xvec(n+1+i)^2/2 - xvec(n+i)^2/2 ...  
                       - xvec(n+i)*(xvec(n+1-i) + kappa + r) + xvec(n+i-1)*(xvec(n+2-i) + kappa ));
            % Lines 42-44 are derived from the HJB equations from LMS's theoretical analysis (Eqn 7 of LMS).
            % Lines 48-49, by comparison, are commented out and are from LMS's function.  Relative to LMS's code, 
            % pivec here is divided by 2c^2.  (Note: In LMS's code, pivec is pre-multiplied (in calibration_EMA_submit.m) by c^2.)  
            % out(n+i) = pivec(n+1+i)-pivec(n+i) + a*(xvec(n+1+i)^2/2 - xvec(n+i)^2/2 ...
            %           - xvec(n+i)*(xvec(n+1-i) + kappa + r) + xvec(n+i-1)*(xvec(n+2-i) + kappa ));               
        else
            % Lines 52-53 are also derived from the HJB equations. A detailed derivation is available from CGLS upon request.
            out(n+i) = (pivec(n+1+i)-pivec(n+i))/2/c^2 + a*(xvec(n+1+i)^2/2 - xvec(n+i)^2/2) ...
        ...
        if i<n
            % Lines 60-61 are also derived from the HJB equations.
            out(n+1-i) = (pivec(n+2-i) - pivec(n+1-i))/2/c^2 + 1/2*(xvec(n-i+2)^2-xvec(n-i+1)^2) ...
                     -xvec(n-i+1)*(xvec(n+i)+kappa+r) + kappa*xvec(n-i+2) + xvec(n+i+1)*xvec(n-i);
            %out(n+1-i) = pivec(n+2-i) - pivec(n+1-i) + 1/2*(xvec(n-i+2)^2-xvec(n-i+1)^2) ...
            %         -xvec(n-i+1)*(xvec(n+i)+kappa+r) + kappa*xvec(n-i+2) + xvec(n+i+1)*xvec(n-i);           
        else
            % Lines 66-67  are also derived from the HJB equations.
            out(n+1-i) = (pivec(n+2-i) - pivec(n+1-i))/2/c^2 + 1/2*(xvec(n-i+2)^2-xvec(n-i+1)^2) ...
                     -xvec(n-i+1)*(xvec(n+i)+kappa+r) + kappa*xvec(n-i+2);
            %out(n+1-i) = pivec(n+2-i) - pivec(n+1-i) + 1/2*(xvec(n-i+2)^2-xvec(n-i+1)^2) ...
            %         -xvec(n-i+1)*(xvec(n+i)+kappa+r) + kappa*xvec(n-i+2);
    ...
    % Lines 73-74  are also derived from the HJB equations.
